...
function openTopMenu($menu){
    ...
    $result .= '<li class="layui-nav-item"><a class="nav-link" href="/Index/index">控制台</a></li>';
    foreach($menu as $item){
        $class = 'nav-link';
        /*if(isset($item['active'])) {
            $class.= ' active';
        }*/
        $result .='<li class="layui-nav-item"><a class="'.$class.'" href="'.$item['url'].'">'.$item['name'].'</a></li>';
    }
    $result = $result . '</ul>';
    return $result;
}
    ...
    if(!empty($clist)){   //如果$id有子栏目，则把每个子栏目处理一次。
        foreach($clist as $k => $v){
            ...
            if(!empty($subcat)){
                ...
                if ($v['url'][0] == '#') {
                    $str=$str.'<li id="menu_'.$v['id'].'" class="'.$active.' layui-nav-item"><a class="" href="'.$v['url'][0].'">'.$v['name'].'</a><ul class="layui-nav-child" style="'.(in_array($v['id'], $menuLevel[0]["self"]) ? 'display: block;' : '').'">'; //如果此栏目有子栏目，那么就是有一个<ul>。
                } else {
 ...
 * @return int
 */
function httpSyncPost($url,$param,$port = '80'){
