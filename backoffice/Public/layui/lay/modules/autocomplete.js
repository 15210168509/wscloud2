;layui.define(['jquery', 'laytpl', 'layer'], function (e) {
    "use strict";
    layui.link('/Public/CSS/autocomplete/autocomplete.css');
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
        layer = layui.layer,
    ...
    job.prototype.pullData = function () {
        ...
        (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
            ...
            success: function (resp) {
                return 1 != eval('resp.' + _config.response.code) ? '' : _config.data[_self.index] = eval('resp.' + _config.response.data), _self.renderData(_config.data[_self.index])
            },
            error: function () {
                hint.error("请求失败")
            },
