...
class RestClient implements \Iterator, \ArrayAccess {
    ...
    public function __construct($options=array()){ // 构造方法
        $default_options = array(
            'headers' => array('Content-type'=>'application/json'), // 定义请求头信息
          //  'headers' => array('Content-type:application/json'),
            'parameters' => array(), // 定义参数
            'curl_options' => array(), // 定义curl
    ...
    public function execute($url, $method='GET', $parameters=array(), $headers=array()){
        ...
        $curlopt[CURLOPT_HTTPHEADER] =array('Content-type:application/json','Expect:');
        ...
        if(strtoupper($method) == 'POST'){
            ...
            logger()->debug( $curlopt[CURLOPT_POSTFIELDS].'curl post json data');
        }
        ...
        if(strtoupper($method) == 'GET' || strtoupper($method) == 'DELETE' ){
            $client->url=$sign->get($client->url);
            //var_dump($client->url);die();
        }elseif(strtoupper($method) == 'POST' || strtoupper($method) == 'PUT' ){
            $client->url=$sign->post($client->url,$parameters);
        }

        //end 
        logger()->debug('request api url : '.$client->url);
  ...
  // add   by zz
   ...
   return json_encode($parameters);
    ...
    public function parse_response($response){
        //logger()->info($response,'curl response from java server');
    ...
    public function decode_response(){
        if(empty($this->decoded_response)){
            ...
            if(!array_key_exists("json", $this->options['decoders']))
