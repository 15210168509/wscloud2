...
class Model {
    ...
    protected function _parseOptions($options=array()) {
        ...
        if(!isset($options['table'])){
            // 自动获取表名
            $options['table']   =   $this->getTableName(); //@by dbn
            $fields             =   $this->fields;
        }else{
     ...
     * @param bool $ignoreValidation 默认为false,如果为true不执行自动验证 @by dbn
     * @return mixed
     */
     public function create($data='',$type='', $ignoreValidation=false) {
        // 如果没有传值默认取POST数据
        if(empty($data)) {
            $data   =   I('post.');
